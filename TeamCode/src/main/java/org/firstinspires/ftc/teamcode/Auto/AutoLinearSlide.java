...
import static org.firstinspires.ftc.teamcode.Util.Constants.MAX_LINEAR_SLIDE_EXTENSION;
import static org.firstinspires.ftc.teamcode.Util.IDs.LEFT_SLIDE_MOTOR_ID;
import static org.firstinspires.ftc.teamcode.Util.IDs.RIGHT_SLIDE_MOTOR_ID;
import static org.firstinspires.ftc.teamcode.Util.Tuning.LINEAR_SLIDE_D;
import static org.firstinspires.ftc.teamcode.Util.Tuning.LINEAR_SLIDE_I;
import static org.firstinspires.ftc.teamcode.Util.Tuning.LINEAR_SLIDE_P;
import static org.firstinspires.ftc.teamcode.Util.Tuning.LINEAR_SLIDE_AUTO_THRESHOLD;
...
public class AutoLinearSlide {
    ...
    public void goToState(int leftVal, int rightVal) {
        ...
        if (desiredState == RobotStates.LinearSlide.MANUEL) {
            ...
            if(Math.abs(customHeight - currentLeftPos) <= LINEAR_SLIDE_AUTO_THRESHOLD) {
                this.leftSlideMotor.setPower(0);
            }

            if(Math.abs(customHeight - currentRightPos) <= LINEAR_SLIDE_AUTO_THRESHOLD) {
        ...
        if(Math.abs(desiredSlideHeight - currentLeftPos) <= LINEAR_SLIDE_AUTO_THRESHOLD) {
            this.leftSlideMotor.setPower(0);
            hasReachedState = true;
        }
        if(Math.abs(desiredSlideHeight - currentRightPos) <= LINEAR_SLIDE_AUTO_THRESHOLD) {
            this.rightSlideMotor.setPower(0);
        }
    }
